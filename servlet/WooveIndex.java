...
public class WooveIndex extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
			...
			if(!cookie.equals("")){
				if(cookie.indexOf("&")>-1){
					...
					for(String trackLast : lastThreeTracks){
						...
						lastThree.add(dbm.getTrackById(Integer.parseInt(theId)).get(0));
					}
				}
				else
					lastThree.add(dbm.getTrackById(Integer.parseInt(cookie.substring(cookie.indexOf("=")+1))).get(0));
			}
		}
		...
		for (int albumT = 0; albumT < aSize; albumT++) {
			...
			TopAlbums toAdd = new TopAlbums(album.getAlbumID() , album.getTitle(), album.getArtist(), track.getCover_img_name());
			topAlbums.add(toAdd);
		}
		...
		for(TrackBean tra: lastTh){
			...
			lastThreeT.add( new TopTracks(tra.getTitle(), alb.getTitle(),
					tra.getArtist(), tra.getCover_img_name(), tra.getInventory_number()));
		}
		...
		ArrayList<TrackBean> genreCookieTracksTmp = dbm.getTrackByGenrePub(genre);
		...
		for(;genreCookieTracks.size()<3;i=(int)(Math.random()*genreCookieTracksTmp.size()))
		...
		if(client!=null){
			if(!client.getLastSearch().equals("")){
				...
				for(;genreTracks.size()<3;j=(int)(Math.random()*genreTracksTmp.size()))
		...
		//Changed from lastThree which are the cookies to lastThreeT which are the 3 most recent tracks
